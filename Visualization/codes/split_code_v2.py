...
def visualization():
    HF_Data = np.load("./HFData.npy",allow_pickle=True)
    X_minMax = np.array(HF_Data[:128])
    
    elecs_xyzDict=np.load("./chnXyzDict.npy",allow_pickle=True)[()] #电极点坐标
    #根据电极坐标筛选seeg数据
    elec_chname = []
    ...
    brain_data=nib.load("./example/mri/orig.mgz") #皮层重建结果
    #brain_data.orthoview()
    aff_matrix=brain_data.header.get_affine() #仿射矩阵
    #print(aff_matrix)
    verl,facel=nib.freesurfer.read_geometry("./example/surf/lh.pial") #左脑皮层表面
    verr,facer=nib.freesurfer.read_geometry("./example/surf/rh.pial") #右脑皮层表面
    ...
    #reCenter_xyzDict=elecs_xyzDict
    #del reCenter_xyzDict['L']
    
    #colormap = ['Greens','Blues','black-white','Purples','blue-red','Greys','pink','summer','winter','jet']
    count = 0
    #colori = 0
    
    pts = mlab.points3d(elec_xyz[:,0], elec_xyz[:,1], elec_xyz[:,2],color=(0,0,0), scale_factor=2)
    #mlab.pipeline.volume(mlab.pipeline.gaussian_splatter(pts))
    #src = mlab.pipeline.scalar_scatter(elec_xyz[:,0], elec_xyz[:,1], elec_xyz[:,2],  X_minMax[:,0])
    
    
    @mlab.animate(delay=10) # 设置延时时间ms
    def anim():
        src = mlab.pipeline.scalar_scatter(elec_xyz[:,0], elec_xyz[:,1], elec_xyz[:,2],  X_minMax[:,0])
        vol = mlab.pipeline.volume(mlab.pipeline.gaussian_splatter(src),vmin=0.5, vmax=0.8)
        
        ctf = ColorTransferFunction()
        ctf.add_rgb_point(100, 1, 1, 0)
        ...
        ms = vol.mlab_source
        for i in range(499):
            #pts.mlab_source.scalars = X_minMax[:,i+1]
